...
4. 詳細規約を確認する: `references/code-guidelines.md`。
5. ツール類（`tflint`, `tfsec` 等）が必要なら `references/tooling.md` を確認する。
6. ローカルで `fmt` → `validate` → `plan` まで通す。
7. 差分を `references/review-checklist.md` で点検する。
8. `apply` は明示合意のうえで実行する（可能なら `plan` 生成物を使って再現性を担保する）。
