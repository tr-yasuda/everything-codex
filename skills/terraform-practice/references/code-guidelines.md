...
- `variables.tf`、`outputs.tf`、`validation` はコントラクト層として扱い、
  module 利用者との約束をここで固定する。

## コントラクト層と実装層

- resource 構成、`locals`、module 内部の組み立ては実装層として扱う。
- コントラクト層を変えずに内部実装を差し替えられる構造を優先する。
- 生成物や補助ファイルを手修正の唯一の正にしない。
  変更は source of truth へ戻して反映する。

## 状態運用とロジックの分離

- backend、workspace、state 操作などの運用上の状態管理と、
  式や変換ロジックを同じ判断に混在させない。
- state の都合で module の公開契約を曖昧にしない。
- 可読性を下げる過剰な `locals` やネストは避ける。
