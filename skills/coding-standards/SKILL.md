...
description: TypeScript、JavaScript、React、Node.js の実装・修正・レビューで、
...
- `tsconfig.json`、`eslint.config.*`、formatter 設定、framework 設定
- 変更が公開 API を含むか、内部実装だけか
- 最小で回せる test / lint / typecheck の単位
...
- test / lint / typecheck のどれを回すべきか決められない
...
TypeScript、JavaScript、React、Node.js のうち、
対象に関係する節だけを適用する。
該当しない技術の規約まで横展開しない。
