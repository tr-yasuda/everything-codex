...
## Keep Design Boundaries Explicit

実装では次の原則を共通で守る。

- 責務の異なる処理を同じ単位へ混在させず、
  関心ごとの境界を明確に保つ
- 状態の保持・更新と、判断・変換・検証のロジックを分離し、
  それぞれを独立に理解・テストできる構造を優先する
- 短さや技巧より、可読性・保守性・変更容易性を優先する
- 変更によって既存テストの保護範囲を狭めない。
  主要経路・失敗系・境界条件のカバレッジを維持する
- API・型・スキーマなどのコントラクト層は先に厳密に定義し、
  実装層はその契約に従う差し替え可能・再生成可能な構造として保つ

生成物や実装詳細に、手修正前提の責務を持たせない。
