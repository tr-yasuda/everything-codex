...
- データ取得、状態更新、表示、変換、検証、副作用を同じ単位へ詰め込まない
- 状態を保持する component / hook と、純粋な判断・変換ロジックを分離する
- UI 層にドメインロジックを埋め込みすぎない。再利用やテストが必要な
  ロジックは関数や module へ切り出す
- 共通化は、可読性と保守性が上がる場合だけ行う

## コントラクト層と実装層

- API、公開関数、export する型、schema、DTO はコントラクト層として
  先に定義する
- コントラクト層の意味を、実装都合で曖昧にしない
- 実装層はコントラクト層に従う差し替え可能な詳細として保つ
- 生成コードや派生実装を手修正の唯一の正にしない。必要な変更は
  契約定義または生成元へ戻して反映する

## テスト保護範囲

- 変更によって既存テストの保護範囲を狭めてはいけない
- 主要経路、失敗系、境界条件のどれを守る変更かを明確にする
- stateful な UI のテストでは、状態遷移と外部との境界を意識する
