...
description: GitHub の Pull Request で、自分が受けたレビューコメントへの
  ...
  で未解決 review thread と修正対象を整理したうえで、修正・返信・resolve・
  再レビュー依頼まで順番に進めたいときに使う。review thread の resolve 状態や
...
1. PR の状態を確認する。
2. コメントを分類して、修正対象と返信対象を分ける。
3. 修正対象を実装する。
4. 返信文をユーザーに提示してから投稿する。
5. 返信が必要な thread を resolve する。
6. 再レビュー依頼は明示された場合だけ行う。
...
返信や修正の前に、PR 全体の状態を把握する。ここで確認が終わる
までは修正に進まない。
...
- この skill では、修正対象の確定後に `github:gh-address-comments`
  へ渡して実装し、その後の返信と resolve は本 skill の流れで行う。
...
返信が必要な thread は、返信を投稿してから resolve する。
